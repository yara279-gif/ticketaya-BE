...
def addmatch (request):
    if request.method == 'POST':
        ...
        if serializer.is_valid(raise_exception=True):
            serializer.save()
            return Response({'msg':"added successfull"},status = status.HTTP_201_CREATED)
...
def deletematch (request, pk):
    if request.method == 'DELETE':
            try:
                ...
                return Response({'error':"Matcher not found"},status = status.HTTP_400_BAD_REQUEST)
...
def search_about_match(request, name=None,team1 =None,team2 =None):
    if request.method == 'POST':
        ...
        if name is None and  team1 is None and team2 is None:
            return Response({'msg':"Name of match or name of any team are required for search"},status = status.HTTP_400_BAD_REQUEST)
        try:
            if name :
            ...
            if serializer.data:
                ...
                return Response({'msg':"No matches found"},status = status.HTTP_404_NOT_FOUND)
        except Match.DoesNotExist as e:
            return Response({'error': f"Match not found {e}"}, status=status.HTTP_404_NOT_FOUND)
    return Response({'msg':"not allowed"},status = status.HTTP_405_METHOD_NOT_ALLOWED)
